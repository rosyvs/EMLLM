...
if REDO:
    ...
    for pID in pIDs: 
        # try:
            ...
            ##### traditional ERP
            # make a new trldict with a separate event type for MW and non MW
            events_to_erp = events_to_model.copy()
            events_to_erp['task+type+mw'] = events_to_erp['task+type'] + '/MW' + events_to_erp['MW'].astype(str)
            annot_to_erp = mne.Annotations(onset=events_to_erp['latency_sec'],
                                                duration=events_to_erp['duration_sec'], 
                                                description=events_to_erp['task+type+mw'])  
            EEG.set_annotations(annot_to_erp)
            trl_to_erp, trldict_to_erp = mne.events_from_annotations(EEG, regexp='.*Fixation*|.*Blink|.*ButtonPress')
            # compute epochs
            epochs = mne.Epochs(EEG, trl_to_erp, event_id=trldict_to_erp, tmin=tmin, tmax=tmax, baseline=(-.1, 0), decim=decimation)
            # average epochs
            evoked={}
            for cond in trldict_to_erp:
                evoked[cond] = epochs[cond].average()
            # plot evoked
            plotconds = []
            fig, ax = plt.subplots(len(trldict_to_erp), 1,figsize=(12, 9))
            for i, cond in enumerate(trldict_to_erp):
                evoked[cond].plot(picks='CPz', axes=ax[i])
            fig.savefig(os.path.join(dir_out, f'{pID}_ERP.png'))
            plotconds = [
                ['reading/FixationWord','reading/Fixation'],
                ['reading/FixationWord/MW0'],
                ['reading/FixationWord/MW1'],
                ['reading/FixationWord','sham/Fixation'],
            ]
            fig, ax = plt.subplots(len(plot_conds), 1,figsize=(12, 9))
            for i,cc in enumerate(plot_conds):
            # rERP_group = mne.grand_average(rERP_ALL)
                plotdict = {k:rERP[k] for k in cc}
                mne.viz.plot_compare_evokeds(plotdict, picks='CPz', axes=ax[i] )
            fig.savefig(os.path.join(dir_out, f'{pID}_ERP_comparisons.png'))

            # save epochs
            epochs.save(os.path.join(dir_out, f'{pID}_epochs-epo.fif'), overwrite=True)
            # save evoked
            mne.write_evokeds(os.path.join(dir_out, f'{pID}_ERP-evk.fif'), [ev for ev in evoked.values()], overwrite=True)
