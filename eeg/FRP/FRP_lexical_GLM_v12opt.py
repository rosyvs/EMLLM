...
#################
# same as 12 but use opt-125m for surprosal
vnum = '12opt'
#################
...
ia_df = pd.read_csv('../../info/ia_label_mapping_opt_surprisal.csv').rename(
    columns={'opt-125m_surprisal_wholetext': 'surprisal'}
)
    ...
    for ch in channels:
        ...
        fig.savefig(
            os.path.join(
                dir_out,
                today,
                f"Group_n=({len(pIDs)}){skip_str}_{contrast_name.replace('/','-')}_{'+'.join(ch)}.png",
            )
        )
    if len(cc) == 1:
        ...
        fig.savefig(
            os.path.join(
                dir_out,
                today,
                f"Group_n=({len(pIDs)}){skip_str}_{contrast_name.replace('/','-')}_topo.png",
            )
        )
