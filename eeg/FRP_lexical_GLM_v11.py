...
#################
# this version: as for V10 but only modeling pages with X assessments (i.e. MW label is not -1)
vnum = 11
#################
...
if REDO:
    ...
    for pID in pIDs: 
        # try:
            ...
            fixations = fixations[fixations['MW'] != -1] # remove unknowns
            ...
            cognitive_covariates = ['MW=1',
                                    "WORD","STOP","PUNC", # these are the binary dummies for fixation types
                                    'WORD_MW=1', # these are the interactions for word fixations
                                    'STOP_MW=1', # these are the interactions for stop words 
                                     'PUNC_MW=1' # these are the interactions for punctuation
            ]
            ...
            plot_conds = [
                ['FRP','FRP_MW=1'],
                ['FRP_WORD', 'FRP_WORD_MW=1'],
                ['FRP','FRP_WORD','FRP_STOP','FRP_PUNC'],
                ['surprisal_WORD','log_word_freq_WORD','relative_word_position_WORD'],
                ['surprisal_WORD','surprisal_WORD_MW=1'],
                ['log_word_freq_WORD','log_word_freq_WORD_MW=1'],
            ]
...
for pID in pIDs:
    ...
    for cond,c in rERP.items():
        if c.nave < 60:
            if ('WORD' in cond or 'WORD_MW=1' in cond): 
...
# %% t test on contrasts
contrast_conds =  [ 
        ['relative_word_position_WORD','relative_word_position_WORD_MW=1'],
'reading/Fixation',
    ['WORD','WORD_MW=1'],
    ['surprisal_WORD','surprisal_WORD_MW=1'],
    ['log_word_freq_WORD','log_word_freq_WORD_MW=1'],
