...
dir_out = '/Volumes/Blue1TB/EEG_processed/FRP_TRF_lexical_v2_rerun3b'
...
if REDO:
    ...
    for pID in pIDs: 
        # try:
            ...
            events_to_model.loc[:,lexical_covariates+gaze_covariates] = events_to_model.loc[:,lexical_covariates+gaze_covariates].apply(lambda x: (x-x.mean())/x.std(), axis=0)
            # fillna in all covatiates as 0, only in covariate cols
            events_to_model.loc[:,lexical_covariates+gaze_covariates+cognitive_covariates] = events_to_model.loc[:,lexical_covariates+gaze_covariates+cognitive_covariates].fillna(0)
...
for pID in pIDs:
    ...
    for cond,c in s.items():
        condnames[cond] = cond
        # baseline correct
        c=c.apply_baseline((-.1, 0), verbose='ERROR')
        # downsample to 100Hz
