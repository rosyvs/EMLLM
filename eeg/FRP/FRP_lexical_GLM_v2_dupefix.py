...
dir_out = '/Volumes/Blue1TB/EEG_processed/FRP_TRF_lexical_v2_dupefix'
...
39, 40, 73, 77, 78, 87,88,93,99, 
    110,115,123,125, 138, 160, 164,167,168, 171,172,173, 178,179,
    # 'EML1_170', 'EML1_175', 'EML1_176', 'EML1_177' are new from edf2asc rerun
    ] 
...
if REDO:
    ...
    for pID in pIDs: 
        # try:
            ...
            # scale and center continuous covariates (lex, gaze) but leave MW/refixations as is
...
for cc in contrast_conds:
    for ch in channels:
        ...
        fig.savefig(os.path.join(dir_out,today, f"Group_n=({len(pIDs)})_{contrast_name.replace('/','-')}_{'+'.join(ch)}.png"))
    if len(cc) == 1:
        evk = mne.grand_average(plotdict[cc[0]])
        fig=mne.viz.plot_evoked_joint(evk, picks=['eeg'], times='peaks',exclude=[])
        fig.savefig(os.path.join(dir_out,today, f"Group_n=({len(pIDs)})_{contrast_name.replace('/','-')}_topo.png"))
