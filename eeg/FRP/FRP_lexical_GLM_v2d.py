...
#################
# same as v2 but here the surprisal and word frequency covariates are set to NA for stop words
vnum = '2d2' 
#################
...
# #####
# pIDs = ['EML1_019', 'EML1_029', 'EML1_035', 'EML1_036', 'EML1_044', 'EML1_051', 'EML1_054', 'EML1_074', 'EML1_101', 'EML1_105', 'EML1_113', 'EML1_130', 'EML1_156', 'EML1_158', 'EML1_170', 'EML1_175', 'EML1_176', 'EML1_177']#TMP
...
if REDO:    
    ...
    for pID in pIDs: 
        # try:
            ...
            events_to_model = events_to_model.drop_duplicates(subset=['eeg_sample_decim'], keep='last') # blinks are way larger than fixations so priotise these
            ...
            cond_counts['Fixation'] = (events_to_model['task+type'].str.contains('Fixation').sum())
            cond_counts['pID'] = pID
            ...
            try:
                ...
                if 'duplicate time points' in str(e):
                    # check how many events are within decimation window
                    diffs = np.diff(trl_to_model[:,0])
                    diffs2 = np.diff(events_to_model['eeg_sample'])
                    close_events = np.where(diffs2<decimation)
                    # get adjacent events 
                    close_events_ = [[e, e+1] for e in close_events[0]]
                    # unlist sublits
                    close_events_ = [item for sublist in close_events_ for item in sublist]
                    # actual rows from events_to_model
                    close_event_rows = events_to_model.iloc[close_events_]
                    print(f'{pID} has {len(close_events)} pairs of events within {decimation}ms of each other')
...
for cc in contrast_conds:
    ...
    if len(cc) == 1:
        ...
        fig.savefig(os.path.join(dir_out, f"Group_n=({len(pIDs)})_{contrast_name.replace('/','-')}_topo.png"))
