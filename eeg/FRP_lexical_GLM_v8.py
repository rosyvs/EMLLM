...
#################
vnum = 8
# this version: as for V7 but no linear effect of inbound saccade amp as this is nonlinear
#################
...
if REDO:
    ...
    for pID in pIDs: 
        # try:
            ...
            word_fixations = fixations[(fixations['IA_ID']!=-1) & (fixations['task']=='reading')]
            word_fixations = word_fixations[word_fixations['stop_word']==0]
            word_fixations = word_fixations[word_fixations['punctuation']==0]

            # remove these from general fixations
            fixations = fixations.drop(word_fixations.index)
            word_fixations['task+type'] = 'reading/FixationWord'
            word_fixations['event_type'] = 'FixationWord'

            ##### MW 
            word_fixations['MW'] = word_fixations['MW'].fillna(-1).astype(int) # third condition coded as -1 for unknown
            # expand to dummy coded cols, one for MW=0 one for MW=1 (intercept is MW=-1)
            word_fixations['MW=0'] = (word_fixations['MW']==0).astype(int)
            word_fixations['MW=1'] = (word_fixations['MW']==1).astype(int)
            ...
            for l in lexical_covariates:
                for c in cognitive_covariates:
                    word_fixations[f'{l}_{c}'] = np.where(word_fixations[c] == 1, word_fixations[l], np.nan)
            ...
            events_to_model = pd.concat([fixations, word_fixations, blinks, task_events])
            ...
            rERP['FRP_MW=0'] = mne.combine_evoked([rERP['MW=0'], rERP['reading/FixationWord']], weights=[1, 1])
            rERP['FRP_MW=1'] = mne.combine_evoked([rERP['MW=1'], rERP['reading/FixationWord']], weights=[1, 1])
            plot_conds = [
                ['FRP_MW=0','FRP_MW=1'],
