...
#################
# this version: Only matched reading and sham, no MW as these pages dont have labels
# model reading and sham as separate events with separate covariates rather than intercept + real reading
vnum = 13
#################
...
ridge_alpha=1
verstr = f'FRP_TRF_readVsham_v{vnum}_alpha{ridge_alpha}_decim{decimation}'
...
if REDO:
    ...
    for pID in pIDs: 
        # try:
            ...
            # remove "sham" from identifier so ia_df merges with it
            events['identifier'] = events['identifier'].str.replace('_Sham', '')
            events['task'] = events['task'].fillna('none') # some are NaN
            events['event_type'] = events['event_type'].fillna('other') # some are NaN
            # filter pages to just sham and matched reading
            sham_identifiers = events['identifier'][events['task'].str.contains('sham')].unique()
            events = events[events['identifier'].isin(sham_identifiers) ]
            ...
            fixations['WORD'] = np.where((fixations['IA_ID']!=-1) & (fixations['stop_word']==0), 1, 0) # for content words & matched sham IAs
            fixations['WORD_READING'] = np.where((fixations['IA_ID']!=-1) & (fixations['stop_word']==0) & (fixations['task'].str.contains('reading')), 1, 0) # for content words in reading
            fixations['WORD_SHAM'] = np.where((fixations['IA_ID']!=-1) & (fixations['stop_word']==0) & (fixations['task'].str.contains('sham')), 1, 0) # for content words in sham
            fixations['SHAM'] = np.where((fixations['task'].str.contains('sham')), 1, 0) # for sham IAs
            fixations['READING'] = np.where((fixations['task'].str.contains('reading')), 1, 0) # for reading
            ##### covariates
            ...
            cognitive_covariates = ['WORD_READING' , 'WORD_SHAM']

            ##### covariates
            # set lexical covariates to NA for all fixations except content words & matched sham IAs
            ...
            lexical_covariates =  lexical_covariates_2 # in this case the interaction term is perfectly collinear with main effect of the original lexical vars so we drop them to prevent signular covariate matrix
            ...
            cond_counts = {}
            for cond in ['WORD_READING', 'WORD_SHAM']:
            ...
            cond_counts['Fixation'] = (events_to_model['event_type']==('Fixation')).sum()
            cond_counts['pID'] = pID
            ...
            rERP['FRP'] = mne.combine_evoked([rERP['reading/Fixation'], rERP['sham/Fixation']], weights=[1, 1])
            rERP['FRP_WORD_READING'] = mne.combine_evoked([rERP['WORD_READING'], rERP['reading/Fixation']], weights=[1, 1])
            rERP['FRP_WORD_SHAM'] = mne.combine_evoked([rERP['WORD_SHAM'], rERP['sham/Fixation']], weights=[1, 1])
            plot_conds = [
                ['FRP_WORD_READING','FRP_WORD_SHAM',],
                ['surprisal_WORD_READING','surprisal_WORD_SHAM'],
                ['log_word_freq_WORD_READING','log_word_freq_WORD_SHAM'],
                ['relative_word_position_WORD_READING','relative_word_position_WORD_SHAM',]
            ]
...
for pID in pIDs:
    ...
    for cond,c in rERP.items():
        if c.nave < 60:
            if ('WORD_READING' in cond or 'WORD_SHAM' in cond): 
...
cond_combinations = {
    ...
    'FRP': ['reading/Fixation','sham/Fixation'],
    'FRP_WORD_READING': ['WORD_READING','reading/Fixation'],
    'FRP_WORD_SHAM': ['WORD_SHAM','sham/Fixation'],
}
condnames = {}
for pID in pIDs:
    s = rERPs[pID]
