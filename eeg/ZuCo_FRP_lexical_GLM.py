...
for pID in pIDs:
    ...
    for b,files in files_by_block.items():
        ...
        if eeg_file and et_file:
            ...
            # make a linear interpolator to get eeg_sample_est from eye latency
            eeg_interpolator = scipy.interpolate.interp1d(events['eye_latency'], events['eeg_latency'], fill_value='extrapolate')
            fix_df['eeg_sample_est'] = eeg_interpolator(fix_df['latency']).astype(int)
            ...
            print(f'word_fixations fix_onset_latency range: {word_fixations["fix_onset_latency"].min()} - {word_fixations["fix_onset_latency"].max()}')  
            # match with words from the sentenceData
            word_fixations.dropna(subset=['fix_onset_latency'], inplace=True)
            word_fixations['eeg_sample_est'] = eeg_interpolator(word_fixations['fix_onset_latency']).astype(int)
            
            # fix_df2 = fix_df.merge(word_fixations, how='left', left_on='eye_latency', right_on='fix_onset_latency')
        ...
        events = pd.concat([b['events'] for b in all_blocks])
        fixations = pd.concat([b['fixations'] for b in all_blocks])
